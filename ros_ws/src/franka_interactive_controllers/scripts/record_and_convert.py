...
def main():
    ...
    base_dir = "/home/tianyu/ncds_ws/non-conservative-DS-controller/ros_ws/NC_CSVS/"
    os.makedirs(base_dir, exist_ok=True)

    bag_name  = os.path.join(base_dir, f"panda_ee_data_.bag")
    merged_csv = os.path.join(base_dir, f"non-conservative_panda_ee_full_.csv")

    # Topics to record → CSV paths under NC_CSVS
    topic_csvs = {
        "/franka_state_controller/F_ext":                                      os.path.join(base_dir, f"fext_.csv"),
        "/nc_passive_ds_impedance_controller/ee_velocity":                     os.path.join(base_dir, f"eevel_.csv"),
        "/franka_state_controller/ee_pose":                                    os.path.join(base_dir, f"eepose_.csv"),
        "/passiveDS/desired_twist":                                            os.path.join(base_dir, f"twist_cmd_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/alpha_":               os.path.join(base_dir, f"alpha_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/beta_r_":              os.path.join(base_dir, f"betar_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/beta_s_":              os.path.join(base_dir, f"betas_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/s_":                   os.path.join(base_dir, f"s_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/sdot_":                os.path.join(base_dir, f"sdot_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/z_":                   os.path.join(base_dir, f"z_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/Dmat":                 os.path.join(base_dir, f"Dmat_.csv"),
        "/nc_passive_ds_impedance_controller/passive_ds/eigval0":              os.path.join(base_dir, f"eigval0_.csv"),
    }
