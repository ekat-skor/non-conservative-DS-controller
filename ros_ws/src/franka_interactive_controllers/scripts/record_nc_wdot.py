...
def main():
    ...
    bag_name = f"/workspace/ros_ws/nc_wdot_params_data_{timestamp}.bag"
    merged_csv = f"/workspace/ros_ws/nc_wdot_params_full_{timestamp}.csv"
    ...
    topic_csvs = {
        ...
        "/nc_passive_ds_impedance_controller/passive_ds/alpha_":               f"/workspace/ros_ws/alpha_{timestamp}.csv",
        ...
        "/nc_passive_ds_impedance_controller/passive_ds/Dmat":                 f"/workspace/ros_ws/Dmat_{timestamp}.csv",
        "/nc_passive_ds_impedance_controller/passive_ds/eigval0":              f"/workspace/ros_ws/eigval0_{timestamp}.csv",
    }
    ...
    try:
        ...
        df_Dmat   = load_and_label(topic_csvs["/nc_passive_ds_impedance_controller/passive_ds/Dmat"],   "Dmat_")
        df_eig0   = load_and_label(topic_csvs["/nc_passive_ds_impedance_controller/passive_ds/eigval0"], "eigval0_")

        df_merged = pd.merge_asof(df_fext, df_vel,  on="time", direction="nearest")
        for df in (df_pose, df_twist, df_alpha, df_beta_r, df_beta_s, df_z, df_Dmat, df_eig0):
