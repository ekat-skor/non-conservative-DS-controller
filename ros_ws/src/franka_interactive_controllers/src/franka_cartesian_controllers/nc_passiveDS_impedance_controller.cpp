    ...
    namespace franka_interactive_controllers
{
  ...
  //|
  NCPassiveDS::NCPassiveDS(const double &lam0, const double &lam1, double s_max, double ds, double dz = 0) : eigVal0(lam0), eigVal1(lam1),
                                                                                                             s_(s_max),
                                                                                                             s_max_(s_max),
  ...
  NCPassiveDS::~NCPassiveDS() {}
  void NCPassiveDS::set_damping_eigval(const double &lam0, const double &lam1)
  {
    if ((lam0 > 0) && (lam1 > 0))
    {
      eigVal0 = lam0;
  ...
  void NCPassiveDS::updateDampingMatrix(const Eigen::Vector3d &ref_vel)
  {
  ...
  Vec NCPassiveDS::gradV(const Vec &x)
  {
    return (damping_eigval + damping_eigval.transpose()) * x * 0.5;
  }

  void NCPassiveDS::update(const Eigen::Vector3d &vel, const Eigen::Vector3d &des_vel)
  {
    ...
    Vec ref_vel_nc = ref_vel - ref_vel_c;
    // compute damping
    updateDampingMatrix(ref_vel);
    // dissipate
    control_output = -Dmat * vel;
    // compute control for conservative part
    control_output += eigVal0 * ref_vel_c;
    // below we add the non-conservative part if allowed
    // non-conservative energy change
    double z = vel.dot(ref_vel_nc);
    if (std::abs(z) < 1e-6)
    {
      z = 0.0;
    }
    control_output += eigVal0 * beta_r_(z, s_) * ref_vel_nc;

    // std::cout<<"value of z:"<< z <<std::endl;

    //  add the non-conservative driving control
    control_output += damping_eigval_(0) * beta_r_(z, s_) * ref_vel_nc;
...
Eigen::Vector3d NCPassiveDS::get_output() { return control_output; }

//*************************************************************************************

bool NCPassiveDSImpedanceController::init(hardware_interface::RobotHW *robot_hw,
                                          ros::NodeHandle &node_handle)
{

  // *********  Subscribers   ********* //
  sub_desired_twist_ = node_handle.subscribe(
      "/passiveDS/desired_twist", 20, &NCPassiveDSImpedanceController::desiredTwistCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  sub_desired_damping_ = node_handle.subscribe(
      "/passiveDS/desired_damp_eigval", 1000, &NCPassiveDSImpedanceController::desiredDampingCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  ...
  {
    ROS_ERROR_STREAM("NCPassiveDSImpedanceController: Could not read parameter arm_id");
    return false;
  }
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no joint_names parameters provided, "
  ...
  {
    ROS_ERROR_STREAM(
        "NCPassiveDSImpedanceController: Error getting model interface from hardware");
    return false;
  }
  try
  {
  ...
  {
    ROS_ERROR_STREAM(
        "NCPassiveDSImpedanceController: Exception getting model handle from interface: "
        << ex.what());
    return false;
  }
  ...
  {
    ROS_ERROR_STREAM(
        "NCPassiveDSImpedanceController: Error getting state interface from hardware");
    return false;
  }
  try
  {
  ...
  {
    ROS_ERROR_STREAM(
        "NCPassiveDSImpedanceController: Exception getting state handle from interface: "
        << ex.what());
    return false;
  }
  ...
  {
    ROS_ERROR_STREAM(
        "NCPassiveDSImpedanceController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i)
  {
    ...
    {
      ROS_ERROR_STREAM(
          "NCPassiveDSImpedanceController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no cartesian_stiffness_setpoint_ctrl parameters provided, "
        "aborting controller init!");
    return false;
  }
  for (int i = 0; i < 6; i++)
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no cartesian_stiffness_grav_comp parameters provided, "
        "aborting controller init!");
    return false;
  }
  for (int i = 0; i < 6; i++)
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no cartesian_stiffness_mode_ parameters provided, "
        "aborting controller init!");
    return false;
  }
  ...
  if (node_handle.getParam("linear_damping_eigenvalues", damping_eigvals))
  {
    ...
    {
      ROS_ERROR(
          "NCPassiveDSImpedanceController: Invalid or no linear_damping_eigenvalues parameters provided, "
          "aborting controller init!");
      return false;
    }
  ...
  if (node_handle.getParam("angular_damping_eigenvalues", damping_eigvals))
  {
    ...
    {
      ROS_ERROR(
          "NCPassiveDSImpedanceController: Invalid or no angular_damping_eigenvalues parameters provided, "
          "aborting controller init!");
      return false;
    }
    for (size_t i = 0; i < 3; ++i)
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no nullspace_stiffness parameters provided, "
        "aborting controller init!");
    return false;
  }
  ...
  {
    ROS_ERROR(
        "NCPassiveDSImpedanceController: Invalid or no external_tool_compensation parameters provided, "
        "aborting controller init!");
    return false;
  }
  ...
  if (node_handle.getParam("q_nullspace", q_nullspace))
  {
    ...
    {
      ROS_ERROR(
          "NCPassiveDSImpedanceController: Invalid or no q_nullspace parameters provided, "
          "aborting controller init!");
      return false;
    }
    for (size_t i = 0; i < 7; ++i)
  ...
  dynamic_server_passive_ds_param_->setCallback(
      boost::bind(&NCPassiveDSImpedanceController::passiveDSParamCallback, this, _1, _2));
  dynamic_server_passive_ds_param_->getConfigDefault(config_cfg);

  return true;
}

void NCPassiveDSImpedanceController::starting(const ros::Time & /*time*/)
{
...
void NCPassiveDSImpedanceController::update(const ros::Time & /*time*/,
...
Eigen::Matrix<double, 7, 1> NCPassiveDSImpedanceController::saturateTorqueRate(
...
void NCPassiveDSImpedanceController::passiveDSParamCallback(
...
void NCPassiveDSImpedanceController::desiredTwistCallback(
...
void NCPassiveDSImpedanceController::desiredDampingCallback(
...
PLUGINLIB_EXPORT_CLASS(franka_interactive_controllers::NCPassiveDSImpedanceController,
